"""Exceptions raised by the public lsmesh API."""
...
if TYPE_CHECKING:
    ...
    from lsmesh.results import MeshAttemptReport


class LsmeshError(RuntimeError):
    """Base class for recoverable lsmesh failures."""


class InvalidGeometryError(LsmeshError):
    """The input geometry cannot be meshed safely."""


class DependencyError(LsmeshError):
    """An optional runtime dependency required for an operation is unavailable."""


class UnsupportedSourceError(TypeError, LsmeshError):
    """The value passed to :func:`mesh` is not a supported mesh source."""


class AutomaticMeshingError(LsmeshError):
...
class MesherNotFoundError(LsmeshError):
